#include <ikfast_pr2_comparisons/comparison_left.h>
#include <time.h>
...
Tester::Tester():counter(0),kdl_c(0),
    ...
    FILE* fp_arm= fopen("pr2_left_arm.cfg", "r");
    if (!fp_arm){ ROS_ERROR("Couldn't open left_arm model file"); }
...
bool Tester::run_ikfast_test(const KDL::Frame& wrist_frame,
    ...
    bool fastik_success = ik_solver.ikLeftArm(wrist_frame, 
...
void Tester::run_ik(const sensor_msgs::JointState& msg){
    ...
    //(32, ('l_shoulder_pan_joint', 0.4325040005647192)),
    //(33, ('l_shoulder_lift_joint', 0.4778479266627372)),
    //(31, ('l_upper_arm_roll_joint', 0.6988121545690104)),
    //(35, ('l_elbow_flex_joint', -1.648332526651105)),
    //(34, ('l_forearm_roll_joint', 2.3324484900413505)),
    //(36, ('l_wrist_flex_joint', -2.0415354562617987)),
    //(37, ('l_wrist_roll_joint', 7.626635690144425)),
    ...
    listener.waitForTransform("/torso_lift_link", "/l_wrist_roll_link", 
                              ros::Time(0), ros::Duration(10));
    listener.lookupTransform("/torso_lift_link", "/l_wrist_roll_link", 
                             ros::Time(0), fk_transform);
    tf::transformTFToKDL(fk_transform, wrist_frame);

    bool fastik_success = run_ikfast_test(wrist_frame, msg.position[31]);
    std::vector<double> bs_angles(7,0);
    bs_angles[2] = msg.position[31];
